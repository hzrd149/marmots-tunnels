...
- `src/index.tsx` — entrypoint: reads env config, builds + starts the server, mounts the Hono routes (`/` group list, `/:groupId` fork-tree overview, `/:groupId/timeline` side-by-side conversations timeline, `/:groupId/:tag` single-epoch page). The `timeline` route is registered before `/:groupId/:tag` so the static segment wins. When a `NOSTR_WHITELIST` is set, the `Auth` gate (`src/auth.ts`) is registered as a `*` middleware **before** the page routes (so it covers all of them), with `/login` (GET page + POST verify) and `/logout` routes added alongside; each page passes the signed-in viewer (as an npub) to its view for the top-bar logout link. With no whitelist the gate is a no-op and the site is public.
- `src/auth.ts` — the optional access gate. `Auth` bundles the whitelist/audience/session config into a `requireAuth()` Hono middleware (bypasses `/login`+`/logout`; valid `nwt` cookie → `c.set("viewer", …)` + continue, else redirect GET to `/login?next=…` or `401`) plus `login`/`logout`/`viewer` handlers. The session cookie is HttpOnly, `SameSite=Lax`, `Secure` auto-detected per request (HTTPS via `x-forwarded-proto`/URL). Pure token logic lives in `src/helpers/web-token.ts` (`decodeToken`/`encodeToken`/`verifyToken`/`tokenExp`): a **Nostr Web Token** is a viewer-signed kind-27519 (`NWT_KIND`) event with `aud`=server npub + `exp` tags, Base64URL-encoded; `verifyToken` checks kind, `verifyEvent` signature, audience, expiry window (±60s skew, capped to the session length), and whitelist membership. Login is client-side (`src/views/login.tsx`) via `window.nostr.js`.
...
TTL — purge groups idle this many hours; unset = retain forever),
`NOSTR_WHITELIST` (comma-separated npubs/hex — when non-empty, gates the UI
behind a NIP-WT login restricted to these keys; unset = public),
`NOSTR_SESSION_HOURS` (login session length, default 24), `PORT`. See
